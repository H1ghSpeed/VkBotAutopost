    ...
    def __publish_post(self, photos: list, text_file):
        ...
        for num, item in enumerate(photos):
            if num == (len(photos)-1):
                ...
                photos_str += f'{item}, '
        req = requests.get(self.urls.url_publish_post(self.group_id, photos_str, text_file)).json()
        if 'error' in req:
            raise OSError(req['error']['error_msg'])

    def public_test(self, photos, text):
        req = requests.post(self.urls.url_publish_post(self.group_id, photos_str, text_file))
    
    def start(self):
        path = f'data/{self.dir_name}'
        posts_id = []
        try:
            for num, item in tqdm(enumerate(os.listdir(os.path.join(path, 'images'))), position=0, leave=False):
